...
2.  **[`rem()`](https://franciscorichter.github.io/amore/reference/rem.md)**
    ...
    conditional-logit backend for case-k-control designs and a
    degenerate-logistic backend for case-1-control, with linear / TV /
    NL / TVNL effects and
...
- **`method = "degenerate"`** — degenerate logistic regression on a
...
fit1 <- rem(~ nl(dyadic.activity), data = w, method = "degenerate")
plot(fit1)            # the fitted smooth panel
```

[`widen_case_control()`](https://franciscorichter.github.io/amore/reference/widen_case_control.md)
reshapes a long case-(k-)control log into the wide `<cov>_ev` /
`<cov>_nv` / `d_<cov>` form the degenerate backend expects (one row per
