...
- *Recovery.* 600 case-1-control strata, where the event in each stratum
  ...
  truth $`\eta = 1.5\,x_1`$ and an **interaction** truth
  $`\eta = 2.5\,x_1 x_2`$. Fit `rem(method = "nn")` and
  `rem(method = "clogit")` on each and compare top-1 concordance (the
  fraction of strata whose observed event scores highest).

**Driver.** `experiments/exp6_nn.R`

**Result — passes.** Analytic and numerical gradients agree to
$`\max|\cdot| = 1.4 \times 10^{-10}`$, confirming the backprop. Under
...
$`2.5\,x_1 x_2`$ — high at the matching-sign corners, low at the
...
Rscript paper/wiki/experiments/exp6_nn.R
```
