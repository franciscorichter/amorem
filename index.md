...
- **[`rem()`](https://franciscorichter.github.io/amore/reference/rem.md)**
  ...
  conditional-logit backend (case-*k*-control) and a degenerate-logistic
  backend (case-1-control) supporting linear / `tv` / `nl` / `tvnl`
  smooth effects and `re()` random effects, plus
