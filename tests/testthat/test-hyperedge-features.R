...
# Coverage for hyperedge_features(): hyperedge-native subrep /
...
test_that("activity stat matches hyperedge_activity()", {
  hl <- fixture()
  feat <- hyperedge_features(hl, stats = "activity")
...
test_that("subrep_<rho> (no underscore l) defaults to receiver-agnostic", {
  hl <- fixture()
  feat <- hyperedge_features(hl, stats = "subrep_1")
...
test_that("dyadic stat on non-dyadic hyperedge log errors out", {
  ...
  expect_error(
    hyperedge_features(hl, stats = "reciprocity_count"),
    "non-dyadic")
})
...
test_that("requested (rho, l) larger than focal row produces NA", {
  ...
  feat <- hyperedge_features(hl, stats = "subrep_2_1")
