...
\emph{plus} a $\tau$-leap approximation for high-rate regimes, a rich
generative endogenous catalogue
(reciprocity counts/binary/exp-decay/time-recent/time-first;
transitivity, cyclic closure, sending/receiving balance counts and
binaries; transitivity time-recent / time-first; per-actor in/out
degree; recency), and a parallel post-hoc feature catalogue exposed
through \texttt{compute\_endogenous\_features()}.

\paragraph{Honest limitations.}
\begin{itemize}
\item \textbf{One-mode constraint for the closure family.}
      Reciprocity-, transitivity-, cyclic-, and balance-style
      statistics require \texttt{identical(senders, receivers)}.
      Bipartite endogenous statistics
      (\texttt{recency}, \texttt{sender\_outdegree},
      \texttt{receiver\_indegree}) already work; the rest is future
      work.
\item \textbf{Generative catalogue still smaller than the post-hoc
      one.}  In particular the \emph{ordered} two-path variants of
      transitivity, exp-decay and time-* variants for cyclic /
      balance, and the \emph{interrupted} reciprocity / triadicity
      family of \citet{juozaitiene2024about} are recognised by
      \texttt{compute\_endogenous\_features()} but not yet by the
      simulator.
...
family of endogenous statistics; (ii) wire ordered-two-path
transitivity, exp-decay and time-* variants for cyclic / balance, and
the interrupted family into the simulator; (iii) full-likelihood path
for $\beta_g$; (iv) a dyad-time-grid covariate API; (v) optional C++
inner loop for high-rate regimes.
