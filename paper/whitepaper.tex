...
\item \textbf{R-only inner loop.}  The scaling benchmark of
      ...
      post-hoc feature engine: \texttt{compute\_endogenous\_features()}
      on the full Manufacturing dataset (82{,}876 events, 167
      actors) takes 89\,s with ten statistics active. Workloads
      that need many feature columns on $\ge 10^5$-event logs are
      better served by careful slicing today, or by an eventual
      C++ inner loop on the roadmap.
\end{itemize}
